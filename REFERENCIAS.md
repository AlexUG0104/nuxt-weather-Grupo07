# Referencias

A continuación se listan las fuentes oficiales y recursos consultados para el desarrollo de esta aplicación:

1. **Nuxt 3 / 4 Documentation - Configuration & Features:**
   *URL:* https://nuxt.com/docs/getting-started/introduction
   *Uso:* Base para la configuración de Nuxt, Server Routes y Runtime Config.

2. **Vue 3 Composition API - Reactivity Fundamentals:**
   *URL:* https://vuejs.org/guide/essentials/reactivity-fundamentals.html
   *Uso:* Implementación de `ref`, `computed` y flujos reactivos.

3. **OpenWeatherMap - Current Weather Data (2.5) API Docs:**
   *URL:* https://openweathermap.org/current
   *Uso:* Estructuración de las peticiones HTTP y mapeo de las respuestas (JSON) y parámetros permitidos.

4. **TypeScript Handbook - Interfaces:**
   *URL:* https://www.typescriptlang.org/docs/handbook/interfaces.html
   *Uso:* Tipado estricto de las respuestas de OpenWeatherMap y estados de la aplicación.

5. **MDN Web Docs - localStorage:**
   *URL:* https://developer.mozilla.org/en-US/docs/Web/API/Window/localStorage
   *Uso:* Persistencia de los datos del historial de búsqueda en el navegador cliente.

6. **MDN Web Docs - CSS Custom Properties (Variables):**
   *URL:* https://developer.mozilla.org/en-US/docs/Web/CSS/Using_CSS_custom_properties
   *Uso:* Creación del sistema de colores y soporte nativo para el modo oscuro (`prefers-color-scheme`).

7. **Vitest - Getting Started:**
   *URL:* https://vitest.dev/guide/
   *Uso:* Configuración del entorno para la ejecución de pruebas unitarias usando la API de Vitest.

8. **Vue Test Utils - Guide:**
   *URL:* https://test-utils.vuejs.org/guide/
   *Uso:* Herramientas para montaje y aserción del comportamiento de componentes de Vue.
